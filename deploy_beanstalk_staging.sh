...
# zip the application
zip -x *.git* -r "${APP_NAME}-local.zip" . 
...
# upload to S3
aws s3 cp ${APP_NAME}-local.zip s3://${S3_BUCKET}/${APP_NAME}-${APP_VERSION}.zip
...
aws elasticbeanstalk update-environment --environment-name "${ENV_NAME}-staging" --version-label "${APP_VERSION}"